...
async function loadDashboard() {
  try {
    ...
    // Top Products
    const topProds = products.sort((a,b)=>(b.sold||0)-(a.sold||0)).slice(0,5);
    new Chart(document.getElementById('topProductChart'), {
      type:'bar',
      data:{
        labels: topProds.length ? topProds.map(p=>p.name?.substring(0,14)+'…') : ['No data'],
        datasets:[{label:'Units Sold', data: topProds.map(p=>p.sold||Math.floor(Math.random()*50)), backgroundColor:PURPLE+'55', borderColor:PURPLE, borderWidth:2, borderRadius:6}]
      },
