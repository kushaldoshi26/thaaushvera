...
@section('title', 'Ashvattha Classic — AUSHVERA')
@section('meta_description', 'Discover the Ashvattha Classic botanical elixir. A singular formulation years in refinement for modern wellness rituals.')
...
<script type="application/ld+json">
{
  ...
  "{{ '@type' }}": "Product",
  "name": "Ashvattha Classic",
  "image": "{{ asset('assets/img/product.jpeg') }}",
  "description": "A concentrated botanical essence designed to support vitality from within.",
  "brand": {
    "{{ '@type' }}": "Brand",
    "name": "AUSHVERA"
  },
        ...
        document.getElementById('reviewForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Note: We don't necessarily reload reviews immediately because the new review is pending
                    if (result.data && result.data.is_approved) {
                        currentPage = 1;
                        loadReviews();
                    }
                } else {
                    ...
                    item.innerHTML = `
                        <div class="review-header-box">
                            <div>
                                ...
                                <div class="review-stars-box">${'★'.repeat(review.rating)}${'☆'.repeat(5 - review.rating)}</div>
                            </div>
